...
UserPromptInst get_prompt_inst(const char* prompt_inst){
    ...
    if(mc_compare_str(prompt_inst, "save", 0))  return (UserPromptInst){USRPROMPT_SAVECONTEXT, 0};
    if(mc_compare_str(prompt_inst, "merge", 0)) return (UserPromptInst){USRPROMPT_MERGECONTEXTS, 1};
    if(mc_compare_str(prompt_inst, "new", 0))   return (UserPromptInst){USRPROMPT_ADDCYM, 3};
